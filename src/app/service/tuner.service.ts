...
export class TunerService {
  ...
  public readonly note$ = merge(
    listen<Note>('tuner::note'),
    this.reset$.pipe(debounceTime(1000)),
  ).pipe(
    ...
    map(p => {
      ...
      p[1].deviation = Math.round(
        (1 - SMOOTH_FACTOR) * p[0].deviation + SMOOTH_FACTOR * p[1].deviation,
      );
    ...
    tap({
      subscribe: () => {
        if (this.subscriptionsCount === 0) {
          invoke('pd_start').catch(console.error);
        }
      ...
      unsubscribe: () => {
        this.subscriptionsCount--;
        if (this.subscriptionsCount === 0) {
          invoke('pd_pause').catch(console.error);
        }
      },
    }),
  );
  ...
  constructor(private tuning: TuningService) {
    effect(() => {
      invoke('pd_base', {f: Number(this.tuning.tuning())}).catch(console.error);
    });
  }
}
