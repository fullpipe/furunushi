...
fn main() {
    let (controls_sender, controls_receiver) = bounded::<drone::Control>(1);
    std::thread::spawn(move || {
        drone::init(controls_receiver);
    });

    std::thread::sleep(std::time::Duration::from_millis(2000));

    controls_sender
        .send(drone::Control::Play(drone::Drone {
            instrument: drone::Instrument::Organ,
            tuning: 440.0,
            midi: 69,
            chorus: true,
            chord: drone::Chord::Minor,
        }))
        .unwrap();
    ...
    controls_sender
        .send(drone::Control::Play(drone::Drone {
            ...
            midi: 69,
            chorus: false,
            chord: drone::Chord::Major,
        }))
        .unwrap();
    // controls_sender.send(drone::Control::Pause).unwrap();

    std::thread::sleep(std::time::Duration::from_millis(2000));

    controls_sender
        .send(drone::Control::Play(drone::Drone {
            instrument: drone::Instrument::Sine,
            tuning: 440.0,
            midi: 69,
            chorus: true,
            chord: drone::Chord::Major,
    ...
    controls_sender.send(drone::Control::Volume(0.5)).unwrap();

    std::thread::sleep(std::time::Duration::from_millis(2000));

    controls_sender.send(drone::Control::Stop).unwrap();
}
